 ...
 */
app.get('/users', (req, res) => { //то, что в кавычках - просто url
 ...
 */
app.get('/users/:id', (req, res) => { //то, что в кавычках - просто url
    const usersJson = fs.readFileSync(usersListPath, 'utf-8');
    const usersData = JSON.parse(usersJson);
...
app.post('/users', (req, res) => {
    ...
    usersData.push({
        id: uniqueID, // "id": 1 + 1
        ...req.body //spread; далее - зарезервированная фнкция для тела запроса, кроме id
    });
